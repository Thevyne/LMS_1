...
def request_book(request, book_id):
    ...
    if not book.available:
        ...
        book_return = BookReturn(book=book, student=request.user)
        if existing_request and book_return.returned == True:
...
def view_requests(request):  
    ...
    requests = BookRequest.objects.filter(is_approved=False, is_rejected=False)
...
def students_in_possession(request):
    if not request.user.is_authenticated or not request.user.is_admin:
        # Redirect to login or handle unauthorized access
        return redirect('login')
    approved_requests = BookRequest.objects.filter(is_approved=True)
    returned_books = BookReturn.objects.all()

    # Fetch all records where the book is currently borrowed and not returned
    borrowed_books = BookReturn.objects.filter(return_date__isnull=True)

    context = {'borrowed_books': borrowed_books}
    return render(request, 'core/students_in_possession.html', context)
...
def reject_request(request, request_id):
    ...
    if book_request.is_approved or book_request.is_rejected:
